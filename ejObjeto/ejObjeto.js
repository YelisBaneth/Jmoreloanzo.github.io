document.getElementById("actualizar").style.display = "none"; //Oculta el bóton de actualiar al inicio
var myArrayEst=[];
var objEst={
    cedula:null,
    nombre:null,
...
var posAct;
comprobarLocalStorage(); // Comprueba si hay datos en localStorage al cargar la página
...
function estado(notaFinal) {
    return (notaFinal >= 3.0 ? "Aprovado" : "Reprobado");
}

function cualitativa(notaFinal) {
    let ecualitativa;
    if(notaFinal >=0 && notaFinal < 2.0) {
        ecualitativa = "Insuficiente";
    }
    else{
        if(notaFinal >= 2.0 && notaFinal < 3.0) {
            ecualitativa = "Deficiente";
        }
        else{
            if(notaFinal >= 3.0 && notaFinal < 3.5) {
                ...
                if(notaFinal >= 3.5 && notaFinal < 4.0) {
                    ecualitativa = "Aceptable";
                }
                else{
                    if(notaFinal >= 4.0 && notaFinal < 4.5) {
                        ecualitativa = "Sobresaliente";
                    }
                    else{
                        if(notaFinal >= 4.5 && notaFinal <= 5.0) {
                            ecualitativa = "Excelente";
                        }
                        else {
                            ecualitativa = "Nota no válida";
                        }
                    }
                }
            }
    ...
    return ecualitativa; // Devuelve la cualitativa correspondiente a la nota final
}
